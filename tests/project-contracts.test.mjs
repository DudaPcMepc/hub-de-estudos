...
test("os cadernos das matérias formam uma árvore privada de pastas, cadernos e páginas", () => {
    ...
    assert.match(frontend, /headerActions\.classList\.toggle\("d-none", itens\.length === 0 \|\| lixeiraAberta\)/);
    assert.match(frontend, /exibirToast/);
    assert.match(frontend, /is-paper-\$\{esc\(item\.estiloFolha\)\}/);
    assert.match(frontend, /salvarPendente/);
    assert.match(frontend, /async function criarPaginaInicial\(caderno\)/);
    assert.match(frontend, /titulo: "Página inicial"/);
    assert.match(frontend, /item\?\.tipo === "notebook" && filhosDe\(item\.id\)\.length === 0/);
    assert.match(frontend, /item\?\.tipo === "notebook" && filhosDe\(item\.id\)\.length > 0/);
    assert.match(frontend, /tipo === "page" && selecionado\(\)\?\.tipo === "page"/);
    assert.match(frontend, /Caderno livre/);
    ...
    assert.match(html, /id="subjectNotebookPageActionsDialog"/);
    assert.match(html, /data-notebook-page-action="delete"/);
    assert.match(html, /data-notebook-page-action="rename"/);
    assert.match(html, /data-notebook-page-action="move"/);
    assert.match(frontend, /subject-notebook-child-meta/);
    assert.match(frontend, /Último acessado/);
    assert.match(frontend, /const paginaAlternativa = irmaos\[indice \+ 1\] \|\| irmaos\[indice - 1\]/);
    assert.match(frontend, /data-notebook-pages-toggle/);
    assert.match(frontend, /id="subjectNotebookPagePaper"/);
    assert.match(frontend, /async function alterarPapelPagina\(estiloFolha\)/);
    assert.match(frontend, /const ultimaPaginaPorCaderno = new Map\(\)/);
    assert.match(frontend, /data-notebook-back/);
    assert.match(frontend, /async function alternarOrganizacao\(forcar\)[\s\S]*?await salvarPendente\(\)/);
    assert.match(frontend, /function renderizarModoOrganizacao\(\)/);
    assert.match(frontend, /subject-notebook-organize-banner/);
    assert.match(frontend, /data-notebook-drag/);
    assert.match(frontend, /pointerdown/);
    assert.match(frontend, /pointermove/);
    assert.match(frontend, /async function moverPorArraste/);
    assert.doesNotMatch(frontend, /data-notebook-order/);
    assert.match(html, /\.subject-notebook-drag-handle[\s\S]*?touch-action: none/);
    assert.match(frontend, /function podeConter/);
    assert.match(frontend, /is-drop-inside/);
    assert.match(frontend, /atualizarRolagemArraste/);
    assert.match(frontend, /btnDesfazerMovimentoCaderno/);
    assert.match(html, /id="btnDesfazerMovimentoCaderno"[^>]*>Desfazer/);
    assert.match(frontend, /data-notebook-rename/);
    assert.match(frontend, /data-notebook-move/);
    assert.match(frontend, /modoDialogo === "rename"/);
    assert.match(frontend, /modoDialogo === "move"/);
    assert.match(html, /id="subjectNotebookNameGroup"/);
    assert.match(html, /id="subjectNotebookParentGroup"/);
    assert.match(frontend, /item\.tipo === "folder" \|\| item\.tipo === "notebook"/);
    assert.match(html, /\.subject-notebook-open[\s\S]*?grid-template-columns: 172px minmax\(0, 1fr\)/);
    assert.match(html, /\.subject-notebook-tree-row\.is-context-active/);
});

test("a lixeira dos cadernos preserva e restaura a hierarquia privada", () => {
    const migration = readProjectFile("supabase/migrations/202609080004_subject_notebook_trash.sql");
    const repository = readProjectFile("src/cloud-core-repository.js");
    const auth = readProjectFile("src/auth.js");
    const frontend = readProjectFile("src/subject-notebooks.js");
    const html = readProjectFile("index.html");

    assert.match(migration, /add column deleted_at timestamptz/i);
    assert.match(migration, /add column deleted_root_id uuid/i);
    assert.match(migration, /with recursive subtree/i);
    assert.match(migration, /set deleted_at = now\(\), deleted_root_id = p_node_id/i);
    assert.match(migration, /restore_subject_notebook_node/i);
    assert.match(migration, /delete_subject_notebook_node_forever/i);
    assert.match(migration, /now\(\) - interval '30 days'/i);
    assert.match(migration, /grant execute on function public\.trash_subject_notebook_node\(uuid\) to authenticated/i);
    assert.match(repository, /\.is\("deleted_at", null\)/);
    assert.match(repository, /export async function carregarLixeiraCadernosMateria/);
    assert.match(repository, /\.rpc\("trash_subject_notebook_node"/);
    assert.match(repository, /\.rpc\("restore_subject_notebook_node"/);
    assert.match(repository, /\.rpc\("delete_subject_notebook_node_forever"/);
    assert.match(auth, /listarLixeira: carregarLixeiraCadernosMateria/);
    assert.match(auth, /restaurar: restaurarDaLixeiraCadernoMateria/);
    assert.match(html, /id="btnLixeiraCadernoMateria"/);
    assert.match(html, /id="btnCadernosMateria"/);
    assert.match(html, /<span>Lixeira<\/span>/);
    assert.match(html, /id="subjectNotebookTrashCount"/);
    assert.match(frontend, /function renderizarLixeira\(\)/);
    assert.match(frontend, /data-notebook-trash-restore/);
    assert.match(frontend, /data-notebook-trash-delete/);
    assert.match(frontend, /data-notebook-trash-empty/);
    assert.match(frontend, /lixeiraAberta = true;[\s\S]*?renderizar\(\);[\s\S]*?await repositorio\.listarLixeira\(materiaAoAbrir\)/);
...
test("o caderno memoriza a última página sem pular a visão geral da matéria", () => {
    ...
    assert.match(migration, /create table public\.user_subject_notebook_positions/i);
    assert.match(migration, /primary key \(workspace_id, user_id, subject_id\)/i);
    assert.match(migration, /foreign key \(last_page_id, workspace_id, user_id, subject_id\)[\s\S]*?on delete cascade/i);
    assert.match(migration, /node\.node_type = 'page'/i);
    assert.match(migration, /force row level security/i);
    assert.match(migration, /user_subject_notebook_positions_select_self[\s\S]*?user_id = \(select auth\.uid\(\)\)/i);
    assert.match(repository, /export async function carregarPosicaoCadernoMateria/);
    assert.match(repository, /export async function salvarPosicaoCadernoMateria/);
    assert.match(repository, /onConflict: "workspace_id,user_id,subject_id"/);
    assert.match(auth, /carregarPosicao: carregarPosicaoCadernoMateria/);
    assert.match(auth, /salvarPosicao: salvarPosicaoCadernoMateria/);
    assert.match(frontend, /function registrarPaginaAtual\(pagina\)/);
    assert.match(frontend, /posicaoAindaNaoDisponivel/);
...
test("trechos do caderno alimentam flashcards, resumos e revisões com contexto", () => {
    ...
    assert.match(html, /await criarNota\(titulo, conteudo, tagFonte \? \[tagFonte\] : \[\]\)/);
    assert.match(html, /function prepararCentralResumos\(\)/);
    assert.match(html, /function alternarFocoResumo\(\)/);
    assert.match(html, /subject-summary-notes/);
    assert.match(html, /subject-summary-overview/);
    assert.match(html, /function textoResumoLegivel\(valor\)/);
    assert.match(html, /function separarFonteResumo\(conteudo\)/);
    assert.match(html, /function alternarListaResumos\(recolher\)/);
    assert.match(html, /id="notaFonteChip"/);
    assert.match(html, /id="notaConteudo" contenteditable="true"/);
    assert.match(html, /data-rich-command="italic"/);
    assert.match(html, /data-rich-command="underline"/);
    assert.match(html, /data-rich-command="ordered-list"/);
    assert.match(html, /id="notaEstiloTexto"/);
    assert.match(html, /data-rich-command="highlight"/);
    assert.match(html, /id="menuCoresDestaqueResumo"/);
    assert.match(html, /data-highlight-color="#ffe58a"/);
    assert.match(html, /data-highlight-color="#d8c4f1"/);
    assert.match(html, /data-highlight-remove/);
    assert.match(html, /function escolherCorDestaqueResumo/);
    assert.match(html, /\[marca=\$\{cor\}\]/);
    assert.match(html, /function editorResumoParaMarkdown/);
    assert.match(html, /function executarComandoResumo/);
    assert.match(html, /function elementoComDestaqueResumo/);
    assert.match(html, /function removerFundosNeutrosResumo/);
    assert.match(html, /function removerDestaqueSelecaoResumo/);
    assert.match(html, /function destaquesSelecionadosResumo/);
    assert.match(html, /if \(estaDestacado\) removerDestaqueSelecaoResumo\(selecao\)/);
    assert.match(html, /destaque\.replaceWith\(\.\.\.destaque\.childNodes\)/);
    assert.match(html, /ativo \? "Remover destaque" : "Destacar"/);
    assert.match(html, /closest\?\.\("blockquote"\) \? "p" : "blockquote"/);
    assert.doesNotMatch(html, /<textarea[^>]*id="notaConteudo"/);
    assert.match(html, /PREFIXO_FONTE_RESUMO/);
    ...
    assert.match(frontend, /function encontrarPaginaPorTitulo\(referencia\)/);
...
test("as telas de estudo restantes evitam controles pequenos no celular", () => {
    ...
    assert.match(html, /\.exam-matrix-table \{ overflow-x: auto; overflow-y: hidden;/);
    ...
    assert.match(html, /\.subject-summary-highlight-palette \{ position: fixed;[^}]*top: auto;/);
